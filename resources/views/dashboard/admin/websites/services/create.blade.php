<x-dashboard.admin.layout.app>
    ...
    <x-slot name="title">
        {{ trans('admin.global.create') . ' - ' . trans('admin.models.services') }}
    </x-slot>
    ...
    <form method="post" action="{{ route('dashboard.admin.websites.services.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="tab-content" id="myTabContent">
                        @foreach(getLanguages() as $language)
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $language->code }}" role="tabpanel" aria-labelledby="{{ $language->code }}-tab">
                                ...
                                <x-input.textarea required="{{ $loop->first ? true : false }}" name="short_description[{{ $language->code }}]" label="admin.global.description"/>
