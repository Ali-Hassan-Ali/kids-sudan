<x-dashboard.admin.layout.app>
    ...
    <x-slot name="title">
        {{ trans('admin.global.create') . ' - ' . trans('admin.models.faqs') }}
    </x-slot>
    ...
    <form method="post" action="{{ route('dashboard.admin.websites.faqs.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="tab-content" id="myTabContent">
                        @foreach(getLanguages() as $language)
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $language->code }}" role="tabpanel" aria-labelledby="{{ $language->code }}-tab">
                                ...
                                <x-input.text required="{{ $loop->first ? true : false }}" name="question[{{ $language->code }}]" label="admin.websites.faqs.question"/>

                                <x-input.textarea required="{{ $loop->first ? true : false }}" name="answer[{{ $language->code }}]" label="admin.websites.faqs.answer" rows="8"/>
